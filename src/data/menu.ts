...
interface SourceItem { name:string; price:string; description:string; diet:Diet; spicy:boolean; hideImage?:boolean; hideAddButton?:boolean }
  ...
  sections:category.sections.map((section, sectionIndex) => ({
    ...
    items:section.items.map((entry, itemIndex):MenuItem => {
      ...
      return {
        ...
        spicy:entry.spicy,
        hideImage:entry.hideImage ?? true,
        hideAddButton:entry.hideAddButton ?? true,
      };
    }),
  })),
}));
